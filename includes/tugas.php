...
if (isset($_POST['batalkan_pengumpulan']) && $role === 'mahasiswa') {
    ...
    // ambil submission tanpa mengasumsikan kolom `nilai` ada
    if (columnExists($conn, 'task_submissions', 'nilai')) {
        $sql_sub = "SELECT nilai, file_path FROM task_submissions WHERE task_id = ? AND mahasiswa_id = ?";
    } else {
        $sql_sub = "SELECT file_path FROM task_submissions WHERE task_id = ? AND mahasiswa_id = ?";
    }
    ...
    $desc_mhs = "Anda telah membatalkan pengumpulan tugas (ID: $task_id).";
...
if (isset($_POST['kirim_tugas']) && $role === 'mahasiswa') {
    ...
    $stmt_cek = $conn->prepare("SELECT 1 FROM tasks t JOIN class_members cm ON t.class_id = cm.class_id WHERE t.id = ? AND cm.mahasiswa_id = ?");
    $stmt_cek->bind_param("ii", $task_id, $user['id']);
    $stmt_cek->execute();
    if ($stmt_cek->get_result()->num_rows == 0) die("Anda tidak memiliki akses ke tugas ini.");
    $stmt_cek->close();
    ...
    if (isset($_FILES['file_tugas']) && $_FILES['file_tugas']['error'] === 0) {
        ...
        if (move_uploaded_file($tmp_name, $file_path)) {
            ...
            $desc_mhs = "Anda telah mengirimkan tugas: " . htmlspecialchars($judul_tugas);
            $stmt_act = $conn->prepare("INSERT INTO activities (user_id, deskripsi, tipe) VALUES (?, ?, 'tugas')");
            $stmt_act->bind_param("is", $user['id'], $desc_mhs);
            $stmt_act->execute();
...
<main class="max-w-4xl mx-auto p-6 md:p-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between border-b border-gray-800 pb-5 mb-8 gap-4 flex-wrap">
        ...
        <form method="GET" action="" class="flex items-center bg-darkbg border border-gray-700 rounded-xl px-3 py-1">
            ...
            <input type="text" name="search" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>" placeholder="Cari judul tugas..." class="bg-transparent text-white text-sm focus:outline-none py-2 w-48">
            <button type="submit" class="ml-2 text-blue-400 hover:text-blue-300"><i class="fas fa-arrow-right"></i></button>
            <?php if(isset($_GET['search']) && !empty($_GET['search'])): ?>
                <a href="tugas.php" class="ml-2 text-gray-500 hover:text-red-400"><i class="fas fa-times"></i></a>
            <?php endif; ?>
        </form>
        ...
        <form method="GET" action="" class="flex items-center bg-darkbg border border-gray-700 rounded-xl px-3 py-1">
            ...
            <?php if(isset($_GET['filter_kelas']) && !empty($_GET['filter_kelas'])): ?>
                <a href="tugas.php" class="ml-2 text-gray-500 hover:text-red-400"><i class="fas fa-times-circle"></i></a>
            <?php endif; ?>
        </form>
        <?php endif; ?>
    </div>
    ...
    <div class="space-y-6">
        <?php
        // =====================================================
        // QUERY TUGAS DENGAN FITUR SEARCH (judul LIKE)
        // =====================================================
                ...
                <?php if ($role === 'mahasiswa'): ?>
                    <?php if ($has_submitted): ?>
                        <div class="bg-green-500/10 border border-green-500/30 rounded-lg p-3 mb-4">
                            <p class="text-green-400 text-sm"><i class="fas fa-check-circle mr-2"></i> Anda sudah mengumpulkan tugas ini pada <?= date('d M Y, H:i', strtotime($submission['submitted_at'])) ?>.</p>
                        </div>
                        ...
                        <?php if ($can_cancel): ?>
                            <form method="POST" class="mt-4">
                                ...
                                <button type="submit" name="batalkan_pengumpulan" class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-bold rounded-xl transition" onclick="return confirm('Yakin ingin membatalkan pengumpulan?')">
                                    <i class="fas fa-trash-alt mr-2"></i> Batalkan Pengumpulan
                                </button>
                            </form>
                        <?php elseif ($submission['nilai'] !== null): ?>
                            <p class="text-gray-400 text-xs mt-2">Tugas sudah dinilai, tidak dapat dibatalkan.</p>
                        <?php elseif ($is_expired): ?>
                            <p class="text-gray-400 text-xs mt-2">Deadline sudah lewat, tidak dapat membatalkan.</p>
                        <?php endif; ?>
                    <?php elseif ($is_expired): ?>
